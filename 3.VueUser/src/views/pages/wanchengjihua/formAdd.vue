			...
			<el-row>
				...
				<el-col :span="8">
					<el-form-item label="计划内容" prop="jihuaneirong">
						<el-input class="list_inp" v-model="form.jihuaneirong" placeholder="计划内容"
							 type="text" 							:readonly="!isAdd||disabledForm.jihuaneirong?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="计划图片" prop="jihuatupian">
						<uploads
							:disabled="!isAdd||disabledForm.jihuatupian?true:false"
							action="file/upload" 
							tip="请上传计划图片" 
							:limit="3" 
							style="width: 100%;text-align: left;"
							:fileUrls="form.jihuatupian?form.jihuatupian:''" 
							@change="jihuatupianUploadSuccess">
						</uploads>
					</el-form-item>
				</el-col>
				<el-col :span="8">
...
<script setup>
	...
	//计划图片上传回调
	const jihuatupianUploadSuccess=(e)=>{
		form.value.jihuatupian = e
	}
	//methods
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
			form.value.wanchengshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='jihuatupian'){
					form.value.jihuatupian = row[x];
					disabledForm.value.jihuatupian = true;
					continue;
				}
	...
	//提交
	const save=()=>{
		if(form.value.jihuatupian!=null) {
			form.value.jihuatupian = form.value.jihuatupian.replace(new RegExp(context?.$config.url,"g"),"");
		}
