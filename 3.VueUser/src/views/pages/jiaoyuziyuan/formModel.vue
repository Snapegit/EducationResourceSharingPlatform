	...
	<div class="app-contain" :style='{"padding":"0px 0","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"86%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">教育分类</div>
					<div  class="info_text" >{{detail.jiaoyufenlei}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">教育资源</div>
					<div class="info_text">
						<el-button class="info_down" v-if="detail.jiaoyuziyuan" @click="downClick(detail.jiaoyuziyuan)">点击下载</el-button>
						<el-button class="info_undown" v-else>暂无</el-button>
					</div>
				</div>
				<div class="btn_view">
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('jiaoyuziyuan','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('jiaoyuziyuan','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('jiaoyuziyuan','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="教育详情" name="first">
				<div v-html="detail.jiaoyuxiangqing"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.jiaoyubiaoti
